...
const RESEARCH_CHART_COLORS = {
  bought: "#22c55e", // green-500
  unlocked: "#3b82f6", // blue-500
  locked: "#94a3b8", // slate-400
};

const Research = ({ actions, upgrades }) => {
  const [data, setData] = useState([]);
  const [researchChartData, setResearchChartData] = useState(null);
  const researchChartRef = useRef(null);
  ...
  useEffect(() => {
    const list = upgrades
      .filter(
        (upgrade) =>
          ...
          ["tech"].includes(upgrade.pool),
      )
      .slice().sort((a, b) => a.price - b.price);

    setData(list);

    // Doughnut: bought, unlocked, locked (tech/research only)
    const all = (upgrades ?? []).filter((u) => u.pool === "tech");
    ...
    setResearchChartData({
      labels: ["Developed", "Available", "Locked"],
      datasets: [
        {
          data: [bought, unlocked, locked],
          backgroundColor: [
            RESEARCH_CHART_COLORS.bought,
            RESEARCH_CHART_COLORS.unlocked,
            RESEARCH_CHART_COLORS.locked,
          ],
        },
      ],
    });
  ...
  return (
    <div className="relative min-w-full">
      ...
      <Attachment
        id="research-status-chart"
        caption="Developed / Available / Locked"
        onChange={handleAttachmentChange}
        onResize={() => {
          researchChartRef.current?.resize();
        }}
        width={300}
        height={300}
        x={750}
        y={100}
        lockAspectRatio
        onClick={() => setSelectedAttachment("research-status-chart")}
        selected={selectedAttachment === "research-status-chart"}
      >
        <Doughnut
          ref={researchChartRef}
          data={researchChartData ?? { labels: [], datasets: [] }}
          options={{
            responsive: true,
            plugins: {
              tooltip: {
